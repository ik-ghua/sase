...
#### 3.4.1 第二刀(Slice 78)revtunnel 零暴露 —— 方向 (A) 锁定(花刚 2026-05-30 定)

**修正第一刀「只换后端」的低估**:Slice77 出站 (b) 是 PoP-TUN+内核 SNAT,**Go 侧不终结 TCP**;revtunnel 是 HTTP/流的应用层承载,故 (c) 需先补「内层包→TCP 流终结」。选 **(A) 内核 REDIRECT 透明代理 + revtunnel 扩原始 TCP 流(不引 gVisor,任意 TCP)**。

锁定的「会错且难改」契约:
- **TCP 终结 = 内核 REDIRECT + SO_ORIGINAL_DST**(非 gVisor):Slice77 PoP-TUN 收到的 allow 包,经 `iptables -t nat PREROUTING -i <ZTNA_TUN> -p tcp -d <connector_app_cidr> -j REDIRECT --to-ports <proxyPort>` → 内核终结 TCP → 本地 listener accept(源 IP=Agent 内层 IP,SO_ORIGINAL_DST 取原 app IP:port)。仅 **connector-backed CIDR** 被 REDIRECT;非 connector 网段仍走 (b) PoP-TUN SNAT——**两出站后端按 dst CIDR 内核分流共存**,appResolver `@connector` 标志与 REDIRECT CIDR 须同源对齐。
- **透明代理处连接级 PEP(必做,安全 crux)**:REDIRECT 流**不经包级 `decide`**(被内核终结,不回 forwardInner),透明代理是其唯一授权检查点——**必须**在 accept 后:① `lookupInnerIP(源 IP)→termSession`,**找不到即 fail-closed 关连接**(无有效 dptunnel 握手→无 byInnerIP 表项→无 principal);② SO_ORIGINAL_DST→`appResolver.Resolve(ts.tenant, dst)`→appKey(只在本租户域);③ 复用抽出的 `decideResource(ts, appKey)`(查吊销+`pep.Decide`,与包路径同一逻辑零行为漂移)→ deny 关连接。
- **stream-mode 的 W9 租户绑定延续**:revtunnel 加 `Hello.Mode`(`http` 缺省向后兼容 / `stream`),stream-mode 注册**完全复用 register 的 W9**(证书 Org==Hello.Tenant);`OpenStream(tenant, app, dst)` 的 key **含 tenant** → A 租户流永远查不到 B 租户 connector。HTTP RoundTrip(W1)/SiteIngress(SD-WAN)走独立 `routes`、**物理分 conn、字节级零改**。
- **byInnerIP B1 不放大**:透明代理复用终结器**同一张** byInnerIP 表(单一真相源),不新建跨租户面;B1 跨租户撞 IP 风险与 Slice77 同源、本刀不放大也不根治(真正硬化=PoP 分配唯一内层 IP / per-tenant 表,后续刀)。
- **nonce 复用无交集**:透明代理只消费已建 session 的 byInnerIP 定位+claims,不重建 session、不动 Seal/Open 计数器;走 revtunnel 的流回程经内核 conntrack(代理 io.Copy)不经 Seal。
- **revtunnel 流 mux**:自写定长帧(OPEN/DATA/CLOSE/RST + streamID),不引 yamux(clean-room 最小依赖;后续需背压再换)。connector `MODE=stream` 收 OPEN→`net.Dial(upstream)`→双向泵。
- **第一刀定界**:仅 TCP(UDP 需 TPROXY,后续);mux 无流控/背压(后续或换 yamux);REDIRECT(非 TPROXY,后续要保留源/UDP 再升级)。零暴露验证:connector+appnet 隔离,**PoP 不直连 app**(`pop-agent` 到 internal-app no route)+ **app 无对外入站**(只经 connector 拨出)。
